...
- `Protein Structure` is a protein-only Foldseek step. Basic Parameters include `Enable Foldseek protein structure similarity`, `Use protein structure mapping TSV`, `Protein structure TSV`, and `ProstT5 model weights`. When a TSV is selected, each non-comment row must be `sequence_id<TAB>structure_file`, and relative structure paths are resolved from the TSV directory. When no TSV is selected, FASTA-only ProstT5/3Di mode requires a local ProstT5 model weights path; oneBuilder does not download that model automatically.
...
- The Foldseek step writes `protein_structure/pairwise_scores.tsv`, `protein_structure/similarity_matrix.tsv`, `protein_structure/distance_matrix.tsv`, `protein_structure/structure_tree.nwk`, and `protein_structure/run_config.json`. Runtime-only `structure_inputs/` and `foldseek_tmp/` directories are intentionally ignored by Git. For CLI/config runs, set `methods.protein_structure.prostt5_model_path` whenever `use_structure_manifest` is `false`.
- For Apptainer/Singularity packaging, include the ProstT5 weights in the image or bind-mount them at runtime, then set `prostt5_model_path` to the path visible inside the container.
