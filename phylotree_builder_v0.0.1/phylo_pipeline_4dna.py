...
class PhylogeneticPipeline:
    def __init__(self, input_file, output_dir="phylo_results"):
        ...
        self.cal_treedist_method_path = "/home/dell/projects/evol_homo_genes_db/scripts/ref_fun/cal_pair_wise_tree_dist.R"
    ...
    def check_and_install_software(self):
        ...
        if not shutil.which('conda'):
            self.logger.error("Conda未找到，请先安装Miniconda或Anaconda")
            sys.exit(1)
        ...
        for package, command in software_packages:
            if not shutil.which(command):
                ...
                try:
                    ...
                    self.logger.warning(f"通过conda安装{package}失败，尝试其他方法")
        ...
        # PHYLIP通常安装在特定目录
        phylip_commands = ['dnadist', 'neighbor', 'dnapars']
        for cmd in phylip_commands:
            if not shutil.which(cmd):
                self.logger.warning(f"PHYLIP命令 {cmd} 未找到，某些功能可能不可用")

        if self.mad_method_path is None:
            self.logger.error(f"MAD rerooting method is not found, please install by-yourself.")
            sys.exit(1)
    ...
    def distance_method(self, phylip_file):
        ...
        try:
            # 1. 计算距离矩阵 (dnadist)
            dnadist_input = "Y\n"  # 使用默认参数
            proc = subprocess.Popen(['dnadist'], stdin=subprocess.PIPE, 
    ...
    def parsimony_method(self, phylip_file):
        ...
        try:
            # 使用dnapars进行简约法分析
            dnapars_input = "Y\n"  # 使用默认参数
            proc = subprocess.Popen(['dnapars'], stdin=subprocess.PIPE,
                                  stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                  text=True)
    ...
    def maximum_likelihood_method(self):
        ...
        try:
            ...
            cmd = [
                ...
                "-s", str(input_file),
                "-m", "MFP",  # 自动选择最佳模型
                "-bb", "1000",  # Ultra-fast bootstrap 1000次
                "-pre", "ml_tree",  # 重新运行
                "--quiet",
                "-redo"
            ]
    ...
    def bayesian_method(self):
        ...
        try:
            ...
            mb_input.append("mcmcp samplefreq=100 printfreq=100 diagnfreq=1000\n")  # FIX: 仅配置
            ...
            for file in Path.cwd().glob("*.con.tre"):
                consensus_tree = file
                sys.path.append(str(Path(__file__).resolve().parents[2] / "ref_fun"))
                from parse_mrb_tree import parse_nexus_tree
        ...
        for i, (tree_file, method) in enumerate( zip(tree_files, methods) ):
            if tree_file and tree_file.exists():
                try:
                    ...
                    axes[i].text(0.5, 0.5, f"Fail to read the tree\n{method}",
                               ha='center', va='center', transform=axes[i].transAxes)
            else:
                axes[i].text(0.5, 0.5, f"树文件不存在\n{method}", 
                           ha='center', va='center', transform=axes[i].transAxes)
    ...
    def generate_summary(self, tree_files, sequences):
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            for method, tree_file in methods:
                ...
                if tree_file and tree_file.exists():
                    f.write(f"    输出文件: {tree_file.resolve().relative_to(self.output_dir.resolve())}\n")
                f.write("\n")
    ...
    def run_pipeline(self):
        ...
        self.logger.info("====Maximum likelihood method complete===========================================================")
        ...
        os.chdir(entry_path)
        self.visualize_trees(rooted_ladd_tree_files)

        self.generate_summary(rooted_ladd_tree_files, sequences)
...
def main():
    ...
    parser.add_argument("-o", "--output", default="phylo_results", 
                       help="输出目录 (默认: phylo_results)")
...
if __name__ == "__main__":
    shutil.rmtree("phylo_results", ignore_errors=True)
